...
/*冒泡排序kml,neon优化*/
void Bubble_sort_neon_kml(float arr[],int n){
	int i,j,k;
	float a=0;
	for(i=0;i<n-1;i++){
		for(j=0;j<n-i-5;j+=4){
			...
			if(vgetq_lane_u32(vectorcompare,0)||vgetq_lane_u32(vectorcompare,1)||vgetq_lane_u32(vectorcompare,2)||vgetq_lane_u32(vectorcompare,3)){
				for (k=0;k<4;k++){
						arr[j+k]=fmin(arr[j+k],arr[j+k+1]);
						arr[j+k+1]=fmax(arr[j+k+1],arr[j+k]); 
                    }
			}
			for(;j<n-i-1;j++){
				arr[j]=fmax(arr[j+1],arr[j]);
				arr[j+1]=fmax(arr[j+1],arr[j]);
			} 
		} 
	}
}
...
/*归并排序带neon,kml*/ 
void merge_neon_kml(float arr[],int left,int mid,int right){
	int nl=mid-left+1;
	int nr=right-mid;
	int i=0,j=0,k=left;
	float leftarr[nl];
    ...
    while(i<nl&&j<nr){
		if(leftarr[i]<=rightarr[j]){
			arr[k]=fmin(rightarr[j],leftarr[i]);
			i++;
		}
		else{
			arr[k]=rightarr[j];
...
void Merge_sort_neon_kml(float arr[],int left,int right){
	if(left>=right) return; 
	int mid=(right+left)/2;
	Merge_sort_neon_kml(arr,left,mid);
	Merge_sort_neon_kml(arr,mid+1,right);
	merge_neon_kml(arr,left,mid,right);
}
/*算法核心在于一次加载四个数据,看是否需要排序,不排序的时候直接四个四个移动*/
...
/*选择排序带neon,kml*/
void  Select_sort_neon_kml(float arr[],int n){
	int i,j,k;
	for(i=0;i<n;i++){
		float min=arr[i];
		int minlocation=i;
...
int main(){
	...
	float *arr_7=copy(arr,N);
	double start7=now();
	Bubble_sort_neon_kml(arr_7,N);
	double end7=now();
	printf("time of bubble kml neon:%.2f\n",end7-start7);
	float *arr_8=copy(arr,N);
	double start8=now();
	Select_sort_neon_kml(arr_8,N);
	double end8=now();
	printf("time of select kml neon:%.2f\n",end8-start8);
	float *arr_9=copy(arr,N);
	double start9=now();
	Merge_sort_neon_kml(arr_9,0,N);
	double end9=now();
	printf("time of merge kml neon:%.2f\n",end9-start9);
	return 0;
}
